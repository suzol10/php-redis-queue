...
class JobManager extends BaseManager
{
  ...
  public function removeJobFromQueue(int|Job $job): bool
  {
    ...
    $removedFromSuccessful = $this->redis->lrem($queue->successful, -1, $job->id());
    $removedFromFailed = $this->redis->lrem($queue->failed, -1, $job->id());
    
    return $removedFromPending === 1 || $removedFromProcessing === 1 || $removedFromSuccessful === 1 || $removedFromFailed === 1;
  }
